...
const Governance = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      const { data: tableData } = await axios('https://iampam-tenants.s3.ap-northeast-2.amazonaws.com/tenants/330886885966/governance/data.json');
      const { data: tableOption } = await axios('https://iampam-tenants.s3.ap-northeast-2.amazonaws.com/tenants/330886885966/governance/schema.json');
      const { data: userTag } = await axios('https://iampam-tenants.s3.ap-northeast-2.amazonaws.com/tenants/330886885966/governance/taguser.json');
      const { data: awsTag } = await axios('https://iampam-tenants.s3.ap-northeast-2.amazonaws.com/tenants/330886885966/governance/tagaws.json');
