...
const NetworkPorts = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      const { data: tableData } = await axios('https://iampam-cloudops-tenants-data.s3.ap-northeast-2.amazonaws.com/tenants/330886885966/network-ports/data.json');
      const { data: tableOption } = await axios('https://iampam-cloudops-tenants-data.s3.ap-northeast-2.amazonaws.com/tenants/330886885966/network-ports/schema.json');
      const { data: userTag } = await axios('https://iampam-cloudops-tenants-data.s3.ap-northeast-2.amazonaws.com/tenants/330886885966/network-ports/taguser.json');
      const { data: awsTag } = await axios('https://iampam-cloudops-tenants-data.s3.ap-northeast-2.amazonaws.com/tenants/330886885966/network-ports/tagaws.json');
  ...
  const saveEditedRow = async (datas: any[]) => {
    await axios({
      method: 'put',
      url: `${import.meta.env.VITE_API_KEY}/network-ports/usertag`,
      data: datas,
    });

    await axios({
      method: 'put',
      url: `${import.meta.env.VITE_API_KEY}/network-ports/awstag`,
      data: datas,
    });
  };
  ...
  return (
    <>
      <div className='panel'>
        ...
        >
          <p className='text-lg'>Network Ports</p>
        </DataTable>
      </div>
    </>
  );
