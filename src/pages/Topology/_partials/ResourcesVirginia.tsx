...
export const ResourcesVirginia = (props: Props) => {
  ...
  const getSvgDimensions = () => {
    const svgElement = document.getElementById('virginiaContainer');
    // @ts-ignore
  ...
  useEffect(() => {
    const data = virginiaData;

    const virginiaContainer = document.getElementById('virginiaContainer');

    while (virginiaContainer?.firstChild) {
      virginiaContainer.removeChild(virginiaContainer.firstChild);
    }

    // Clear previous content
    if (virginiaContainer?.innerHTML) virginiaContainer.innerHTML = '';
    ...
    // @ts-ignore
    virginiaContainer.appendChild(regionGroup);
    ...
    for (const [key, resource] of Object.entries(data.resources)) {
      ...
      // @ts-ignore
      virginiaContainer.appendChild(resourceGroup);
      resourceIndex++;
    }
  });
  return (
    <div className='resources-box'>
      <svg id='virginiaContainer'></svg>
    </div>
  );
};
