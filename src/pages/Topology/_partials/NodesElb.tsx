...
import elbJson from '../data/nodes-elb.json';

export const NodesElb = () => {
  ...
  useEffect(() => {
    const data = elbJson;
    const elbContainer = document.getElementById('elbContainer');
    const tooltip = document.getElementById('tooltip');

    // Clear previous content
    if (elbContainer) elbContainer.innerHTML = '';

    // Get width and height from CSS
    const width = parseInt(getCssPropertyValue('#elbContainer', 'width'));
    const height = parseInt(getCssPropertyValue('#elbContainer', 'height'));
    const padding = 20;
    ...
    distinctCidrs.forEach((cidr, cidrIndex) => {
      ...
      // @ts-ignore
      elbContainer.appendChild(cidrGroup);
    });
  });

  return (
    <div className='nodes-box'>
      <svg id='elbContainer'></svg>
    </div>
  );
};
