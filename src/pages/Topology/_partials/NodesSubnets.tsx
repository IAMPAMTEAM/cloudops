...
import subnetsJson from '../data/nodes-subnets.json';
// import '../css/nodes.css';

export const NodesSubnets = () => {
  ...
  useEffect(() => {
    ...
    if (subnetsContainer) subnetsContainer.innerHTML = '';

    // Get width and height from CSS
    const width = parseInt(getCssPropertyValue('#subnetsContainer', 'width'));
    const height = parseInt(getCssPropertyValue('#subnetsContainer', 'height'));
    const padding = 20;
    ...
    distinctCidrs.forEach((cidr, cidrIndex) => {
      ...
      subnetsContainer.appendChild(cidrGroup);
    });
  });

  return (
    <div className='nodes-box'>
      <svg id='subnetsContainer'></svg>
    </div>
  );
};
