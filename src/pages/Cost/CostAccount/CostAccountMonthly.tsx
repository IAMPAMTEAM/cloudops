...
const CostAccountMonthly = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      const { data: tableData } = await axios('https://iampam-cloudops-tenants-data.s3.ap-northeast-2.amazonaws.com/tenants/330886885966/cost/account-monthly/data.json');
      const { data: tableOption } = await axios('https://iampam-cloudops-tenants-data.s3.ap-northeast-2.amazonaws.com/tenants/330886885966/cost/account-monthly/schema.json');
      const { data: userTag } = await axios('https://iampam-cloudops-tenants-data.s3.ap-northeast-2.amazonaws.com/tenants/330886885966/cost/account-monthly/taguser.json');
      const { data: awsTag } = await axios('https://iampam-cloudops-tenants-data.s3.ap-northeast-2.amazonaws.com/tenants/330886885966/cost/account-monthly/tagaws.json');
    ...
    <>
      ...
      <div className='grid lg:grid-cols-4 gap-[1.2rem] mt-[1.2rem]'>
        <div className='panel lg:col-span-2'>
          <p className='text-[16px] font-semibold'>Last Month</p>
...
export default CostAccountMonthly;
