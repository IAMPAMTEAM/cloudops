...
import CostRegionTop from '@/assets/images/Cost_TopRegions.svg';
...
const CostRegionDaily = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      const { data: tableData } = await axios('https://iampam-cloudops-tenants-data.s3.ap-northeast-2.amazonaws.com/tenants/330886885966/cost/region-daily/data.json');
      const { data: tableOption } = await axios('https://iampam-cloudops-tenants-data.s3.ap-northeast-2.amazonaws.com/tenants/330886885966/cost/region-daily/schema.json');
  ...
  return (
    <>
      <img src={CostRegionTop} className='w-full h-full mb-4' />
      <div className='panel'>
        <DataTable
...
export default CostRegionDaily;
